...
export function createServer() {
  const server = new FastMCP({
    ...
    instructions: [
      ...
      "  1. 新需求/已有代码 → 先 discover,输出业务入口、调用链、业务规则、例子、证据来源和真正无法从材料判断的待确认问题",
      ...
      "若准备编写或修改 .feature → 先调用 guide({ topic: \"gherkin-official\" }) 查看官方 Gherkin 结构和 harness 模板。",
      "若用户担心 AI 写烂代码 → 配置 .harness/lint/rules.yaml 或 commands.lint 后运行 lint(),不要用 check 代替代码质量门禁。",
      "",
      "── Feature 文件协议(所有项目硬性遵守) ──",
      "  • 所有新建/修改的 spec_dir .feature 文件按 harness.yaml language 校验;缺省 zh-CN,也可配置 en",
      "  • 头部必须有元数据注释:",
      ...
      "  • 来源追溯写在 .harness/source-map.yaml;不要在 feature 注释里重复维护来源",
      "  • source-map.yaml 可选;存在时 check 校验 current/timeline、日期顺序和来源文件存在性",
      "  • 禁止只写“接口成功 / 返回完整内容 / 状态码 200”这类浅断言",
      "",
      "── 自审触发协议(不要混用) ──",
      "  • Feature Discovery Review 触发于 discover,用于人工确认前",
      "  • Discovery 提问前必须先查 PRD、代码、配置、枚举、注释、调用链和现有测试;能从材料回答的先写入已确认规则,不要作为待确认问题",
      "  • 待确认问题只保留材料冲突、缺少产品语义、存在预留状态但无触发路径等无法从证据链解决的事项",
      "  • Feature Contract Review 只审 feature 契约,不审 step 实现",
      "  • 触发: contract 写入 feature 后;手动修改 .harness 下的 .feature 后",
      ...
      "",
      "── 目录组织 ──",
      "  • harness.yaml 推荐固定 spec_dir: .harness,不要让 AI 自己发明 specs 目录",
      ...
      "  • 业务能力 → spec_dir/features/<target>/<domain>/<业务动作>.feature,不要直接放在 spec_dir/<domain>",
      ...
      "  • BDD step definitions、runner config、测试代码和报告不写进 spec_dir;它们属于宿主项目 test/build 输出",
