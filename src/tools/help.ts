...
import { SOURCE_MAP_SCHEMA_HELP } from "../sources.ts";
import {
  findToolHelp,
  HELP_TOPICS,
  TOOL_CATALOG,
...
function renderWorkflow(): string {
  return [
    ...
    "  sources 负责沉淀 PRD、人工确认、代码推断或其他来源;source-map.yaml 可选记录 capability/rule 与来源时间线。",
  ].join("\n");
}
...
function renderDiscoverGuide(): string {
  return [
    ...
    "  - 业务例子: 成功、失败、边界、异常",
    "  - 待确认问题: 只能是查完 PRD、代码、配置、枚举、注释、调用链和现有测试后仍无法判断的问题",
    "  - 证据来源: PRD / 用户提供 / 代码推断 / 现有测试 / 人工确认",
    "  - 来源文档: 进入 feature 的业务承诺应沉淀到 .harness/sources/YYYY-MM-DD-xxx.md",
    "",
    "提问规则:",
    "  - 不要为了凑问题而问;先尽最大可能从材料回答",
    "  - 配置值、枚举列表、是否有重试/超时/锁/状态流转等问题,必须先查配置类、枚举、调用链和测试",
    "  - 能从代码或文档确认的内容,写入业务规则并在 evidence 中标明来源",
    "  - 只有材料冲突、业务语义缺失、存在状态但找不到触发路径、或实现行为可能只是历史偶然时,才列入 questions",
    "",
    "待确认问题推荐写法:",
    "  - 已查: PracticeStatus enum、submit/correct/timeoutScan 调用链、定时任务目录",
    "  - 仍需确认: DISCARDED 状态存在但未找到写入路径,它是预留状态还是由外部任务触发?",
    "",
...
function renderContractGuide(): string {
  return [
    ...
    "  - 来源追溯写在 source-map.yaml,不要塞进 feature 注释里",
    "",
    "```gherkin",
    "# language: zh-CN",
...
function renderOfficialGherkinGuide(): string {
  return [
    ...
    "```",
    "",
    "来源追溯写在 .harness/source-map.yaml,不是 feature 注释:",
    "```yaml",
    "version: 1",
    "capabilities:",
    "  api.order.create:",
...
function renderSourcesGuide(): string {
  return [
    "sources: 业务来源追溯",
    "",
    "来源文档统一放在 .harness/sources/。它可以是 PRD、人工确认、会议纪要、工单、代码推断或现有测试推断。PRD 不是必需的,但进入 feature 的业务承诺应该能追到来源。",
    "",
    "目录:",
    ...
    "```",
    "",
    "来源关系写在 .harness/source-map.yaml。它是可选治理文件,用于记录 capability/rule 当前生效来源和时间线:",
    "",
    SOURCE_MAP_SCHEMA_HELP,
    "",
    "规则:",
    ...
    "  - contract 不因为缺 source-map.yaml 拒绝 feature;check 只拒绝写错的 source-map.yaml",
  ].join("\n");
}
...
function renderHarnessYamlGuide(): string {
  return [
    "harness.yaml: 项目接入配置",
    "",
    "harness.yaml 放在宿主项目根目录,用于声明 .harness 契约目录、BDD runner、项目 lint 命令和可选 AI 提示。",
    "",
...
function renderCheckGuide(): string {
  return [
    ...
    "  - source-map.yaml 格式错误、文件不存在或 timeline 倒序",
