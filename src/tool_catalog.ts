...
export const TOOL_CATALOG: ToolCatalogEntry[] = [
  {
    ...
    commonArgs: ["topic: overview | workflow | tools | feature | check | flow | <tool_name>", "raw"],
    example: 'help({ "topic": "feature" })',
    notes: [
  ...
  },
  {
    name: "info",
    group: "发现项目",
    description: "查看当前项目的 harness 接入状态、目录、能力统计、tag 和 verify 配置。",
  ...
  },
  {
    name: "doctor",
    group: "发现项目",
    description: "静态自检 harness 接入:配置、目录、capability 元数据、feature 质量和 verify 报告配置。",
  ...
  },
  {
    name: "verify",
    group: "验证",
    description: "执行 harness.yaml 里的 verify.cmd,解析报告并返回通过/失败摘要。",
    useWhen: "改完业务、测试或代码后确认业务验证是否通过。",
    commonArgs: ["path", "capability", "include_diff", "raw"],
    example: 'verify({ "capability": "billing.createOrder" })',
    notes: ["默认不返回 git diff。失败后 AI 可以自行查看 diff 和测试日志。"],
  },
  {
    name: "run",
    group: "验证",
    description: "执行 commands.run 普通业务验证命令,未配置时兼容回退到 verify。",
    useWhen: "项目区分普通业务验证和 capability verify 时。",
    commonArgs: ["path", "raw"],
    example: "run({})",
    notes: ["适合接宿主项目已有测试命令。"],
  },
  {
    ...
    notes: ["没有 name 时只列出 flow。传 name 时执行 commands.flow。"],
  },
  {
    name: "check",
    group: "验证",
