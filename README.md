...
- 待确认问题：查完 PRD、代码、配置、枚举、注释、调用链和现有测试后仍无法判断的业务点。

待确认问题不是“AI 没查到就问人”。AI 必须先尽最大可能从材料回答：

- 配置值、枚举列表、是否有重试/超时/锁/状态流转等问题，先查配置类、枚举、调用链和测试。
- 能从代码或文档确认的内容，写入业务规则并在 evidence 中标明来源。
- 只有材料冲突、业务语义缺失、存在状态但找不到触发路径、或实现行为可能只是历史偶然时，才列入待确认。
